"""OpenAI-compatible shim that forwards Hermes requests to `claude --acp`.

This adapter lets Hermes treat the Claude Code ACP server as a chat-style
...
class ClaudeACPClient:
    ...
    def _create_chat_completion(
        ...
        **_: Any,
        ...
        # Streaming is not supported by the ACP subprocess protocol —
        # raise a recognized error so Hermes switches to non-streaming mode
        # via its existing _disable_streaming fallback path.
        if stream:
            raise RuntimeError("stream is not supported by claude-acp")
        ...
        # timeout may be a number, None, or an httpx.Timeout object.
        # Extract a usable float in all cases.
        if timeout is None:
            timeout_value = float(_DEFAULT_TIMEOUT_SECONDS)
        else:
            try:
                timeout_value = float(timeout)
            except (TypeError, ValueError):
                # httpx.Timeout exposes .read / .connect attributes; prefer read
                read_val = getattr(timeout, "read", None)
                if read_val is None:
                    read_val = getattr(timeout, "connect", None)
                try:
                    timeout_value = float(read_val) if read_val is not None else float(_DEFAULT_TIMEOUT_SECONDS)
                except (TypeError, ValueError):
                    timeout_value = float(_DEFAULT_TIMEOUT_SECONDS)
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            while time.time() < deadline:
                ...
                if "error" in msg:
                    ...
                    raise RuntimeError(
                        f"Claude ACP {method} failed: {err.get('message') or err}"
                    )
