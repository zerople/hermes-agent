...
def _model_flow_claude_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set HERMES_CLAUDE_ACP_COMMAND or CLAUDE_CLI_PATH if Claude CLI is installed elsewhere.")
        return

    effective_base = creds.get("base_url") or effective_base
    model_list = _PROVIDER_MODELS.get("claude-acp", []) or _PROVIDER_MODELS.get("anthropic", [])
...
"""
    ...
    chat_parser.add_argument(
        "--provider",
        choices=["auto", "openrouter", "nous", "openai-codex", "copilot-acp", "claude-acp", "copilot", "anthropic", "gemini", "huggingface", "zai", "kimi-coding", "kimi-coding-cn", "minimax", "minimax-cn", "kilocode", "xiaomi"],
        default=None,
